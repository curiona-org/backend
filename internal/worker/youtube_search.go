...
func (w *worker) EnqueueSearchYoutubeExternalResources(ctx context.Context, payload SearchYoutubeExternalResourcesInput) error {
	ctx, span := w.tracer.Start(ctx, "(*worker.EnqueueSearchYoutubeExternalResources)")
	defer span.End()
...
func (w *worker) searchYoutubeExternalResources(ctx context.Context, task *asynq.Task) error {
	traceCtx, span := w.tracer.Start(ctx, "(*worker.searchYoutubeExternalResources)")
	defer span.End()
